           ...
           while ($row_a= $result->fetch_assoc()) {
              ...
              <tr>
                ...
                <td class="text-center">
                  ...
                  if($level_user==1){
                  echo'
                  <a href="#modalEdit" class="btn btn-warning btn-md enable-tooltip" title="Edit" data-toggle="modal"';?> onclick="getElementById('txtid').value='<?PHP echo $row_a['user_id'];?>';getElementById('txtnama').value='<?PHP echo $row_a['fullname'];?>';getElementById('txtuser').value='<?PHP echo $row_a['username'];?>';getElementById('txtemail').value='<?PHP echo $row_a['email'];?>';getElementById('txtlevel').value='<?PHP echo $row_a['level'];?>';"><i class="fa fa-pencil-square-o"></i></a><?PHP }
                  else{
                      // cek level 2 berdasarkan id login
                      if($user_id ==$SESSION_ID){
                       echo'<a href="#modalEdit" class="btn btn-warning btn-md enable-tooltip" title="Edit" data-toggle="modal"';?> onclick="getElementById('txtid').value='<?PHP echo $row_a['user_id'];?>';getElementById('txtnama').value='<?PHP echo $row_a['fullname'];?>';getElementById('txtuser').value='<?PHP echo $row_a['username'];?>';getElementById('txtemail').value='<?PHP echo $row_a['email'];?>
                       ';getElementById('txtlevel').value='<?PHP echo $row_a['level'];?>';"><i class="fa fa-pencil-square-o"></i></a><?PHP
                      }else{
                         echo'<button class="btn btn-sm btn-warning access-failed" title="Ubah"><i class="fa fa-pencil-square-o"></i></button>';
                      }
                  }
                  
                  if($level_user==1){
                    echo'
                      <button data-id="'.epm_encode($row_a['user_id']).'" class="btn btn-md btn-danger delete" title="Hapus"><i class="fa fa-trash-o"></i></button>';
                    }else{
                    echo'
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="form-group">
            ...
            <small class="text-muted">Kosongkan jika tidak ingin mengisi email.</small>
        </div>
